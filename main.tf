...
resource "azurerm_resource_group" "rg" {
    ...
    location = "westus2"
    tags      = {
      Environment = "Developmenty"
    }
}

#Create virtual network
resource "azurerm_virtual_network" "vnet-dev-network" {
    name                = "vnet-dev"
...
resource "azurerm_subnet" "subnet" {
  name                 = "snet-dev"
  resource_group_name  = azurerm_resource_group.rg.name
  virtual_network_name = azurerm_virtual_network.vnet-dev-network.name
  address_prefix       = "10.0.0.0/24"
}

# Create public IP
resource "azurerm_public_ip" "publicip" {
  name                = "pip-vmterraform-dev"
  location            = "westus2"
  resource_group_name = azurerm_resource_group.rg.name
